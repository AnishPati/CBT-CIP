...
int main()
{
    ...
    printf("WELCOME TO BANK ACCOUNT SYSTEM OF ANISH PATI");
    ...
    case 1:
        system("cls");
        printf("\n\n USERNAME 50 CHARECTERS MAX!!");
        printf("\n\n PASSWORD 50 CHARECTERS MAX!!");
        CreateBankAccount();
        break;
...
void login(void){
    ...
    while(fread(&u1,sizeof(u1), 1,file1)){
...
void loginsu(void)
{
    ...
    struct accountDetails u1;
    system("cls");
    printf("Fetching account details based on your input......\n");
    for(i=0;i<20000;i++){
        i++;
        i--;
    }
...
void display(char user1[])
{
    ...
    while(fread(&u1,sizeof(u1),1,file1)){
        if(strcmp(user1,u1.username) == 0){
            ...
            printf("FATHERER's NAME's..%s %s",u1.fathername,u1.lname);

            displayCoordinates(55,14);
            printf("MOTHERER's NAME's..%s %s",u1.mothername,u1.lname);
...
void SendMoney(void)
{
    ...
    while(fread(&u1,sizeof(u1), 1, file2))
    {
     if(strcmp(reciever, u1.username)==0)
        {
            strcpy(m1.username, u1.username);
            strcpy(m1.userpersonfrom, sender); 
        }
    }
...
void CheckBankBalance(char user2[])
{
    ...
    while(fread(&m1,sizeof(m1),1,file1)){
        if(strcmp(user2,m1.username)==0){
            displayCoordinates(k, ++l);
            printf("%d",i);
            i++;
            displayCoordinates(m, ++n);
